...
| mobile-composer-menu | ≤767px: composer controls collapse into a single model-indicator trigger (inline chip row not rendered) | yes | n/a | yes | yes | implemented | unit: useIsMobile=true → menu.action present, provider/reasoning.action absent; browser-harness @390px on :5174 shows lone `⬡ Opus ⌄`, provider.action False |
