...
import type { TunnelGateway } from "../claude-pty/tunnel-gateway"
import { createAskUserQuestionTool } from "./tools/ask-user-question"
import { createExitPlanModeTool } from "./tools/exit-plan-mode"
import { createReadTool } from "./tools/read.adapter"
import { createGlobTool } from "./tools/glob.adapter"
import { createGrepTool } from "./tools/grep.adapter"
import { createBashTool } from "./tools/bash.adapter"
import { createEditTool } from "./tools/edit.adapter"
import { createWriteTool } from "./tools/write.adapter"
...
export function buildKannaMcpTools(args: KannaMcpArgs): SdkMcpToolDefinition<any>[] {
  ...
  const tools: SdkMcpToolDefinition<any>[] = [
    ...
    tool(
      ...
      async (input) => {
        ...
        if (outcome.status === "no_tunnel" || outcome.status === "error") {
          return {
            content: [{ type: "text" as const, text: outcome.reason }],
            isError: true,
          }
  ...
  if (builtinShimsEnabled && args.toolCallback) {
    ...
    function registerShim<I>(shim: {
      ...
      handler: (input: I, ctx: import("./tools/tool-callback-shim").ToolHandlerContext) => Promise<import("./tools/tool-callback-shim").ToolHandlerResult>
    }) {
      tools.push(
        tool(
          shim.name,
