...
export async function verifyPtyAuth(args: {
  ...
  return {
    ok: false,
    error: "No OAuth pool token supplied. PTY mode is OAuth-only and requires an OAuth-pool token configured in Tinkaria settings; API keys and the local `claude /login` keychain path are not used.",
  }
}
